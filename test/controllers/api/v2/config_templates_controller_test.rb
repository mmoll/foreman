...
class Api::V2::ConfigTemplatesControllerTest < ActionController::TestCase
  ...
  test "should update valid" do
    ProvisioningTemplate.any_instance.stubs(:valid?).returns(true)
    put :update, params: { :id              => templates(:pxekickstart).to_param, :config_template => { :template => "blah" } }
    assert_response :ok
  end
  ...
  test "should update associated operating systems with unwrapped parameters" do
    ...
    ProvisioningTemplate.any_instance.stubs(:valid?).returns(true)
    put :update, params: { :id => tpl.to_param, :operatingsystem_ids => [os.to_param] }
    assert_response :ok
  ...
  test "should add audit comment" do
    ...
    ProvisioningTemplate.any_instance.stubs(:valid?).returns(true)
    put :update, params: { :id              => templates(:pxekickstart).to_param, :config_template => { :audit_comment => "aha", :template => "tmp" } }
    assert_response :success
  ...
  test 'should clone template' do
    ...
    post :clone, params: { :id => original_config_template.to_param, :config_template => {:name => 'MyClone'} }
    assert_response :success
    template = ActiveSupport::JSON.decode(@response.body)
  ...
  test 'clone name should not be blank' do
    post :clone, params: { :id => templates(:pxekickstart).to_param, :config_template => {:name => ''} }
