...
class Api::V2::PtablesControllerTest < ActionController::TestCase
  ...
  test "should add audit comment" do
    ...
    ptable = FactoryGirl.create(:ptable)
    put :update, params: { :id => ptable.to_param, :ptable => { :audit_comment => "aha", :template => "tmp" } }
    assert_response :success
  ...
  test 'should clone template' do
    ...
    post :clone, params: { :id => original_ptable.to_param, :ptable => {:name => 'MyClone'} }
    assert_response :success
  ...
  test 'clone name should not be blank' do
    post :clone, params: { :id => FactoryGirl.create(:ptable).to_param, :ptable => {:name => ''} }
    assert_response :unprocessable_entity
  end
end
