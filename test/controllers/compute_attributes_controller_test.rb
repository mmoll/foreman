...
class ComputeAttributesControllerTest < ActionController::TestCase
  ...
  test "should create compute_attribute" do
    assert_difference('ComputeAttribute.count') do
      ...
      post :create, params: { :compute_profile_id => @compute_profile.to_param, :compute_attribute => { :compute_resource_id => @compute_resource.to_param, :compute_profile_id => compute_profiles(:three), :vm_attrs => {"flavor_id"=>"t2.medium"} } }, session: set_session_user
    end
  ...
  test "should update compute_attribute" do
    put :update, params: { :id => @set, :compute_profile_id => @compute_profile.to_param, :compute_attribute => { :compute_resource_id => @set.compute_resource_id, :compute_profile_id => @set.compute_profile_id, :vm_attrs => {"flavor_id"=>"t2.medium"} } }, session: set_session_user
