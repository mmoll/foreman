...
class AuthSourceLdapsControllerTest < ActionController::TestCase
  ...
  context 'user with viewer rights' do
    ...
    test 'should fail to edit authentication source' do
      get :edit, params: { :id => AuthSourceLdap.unscoped.first.id }, session: set_session_user(users(:one))
  ...
  test 'organizations/locations can be assigned to it' do
    ...
    put :update, params: { :id => AuthSourceLdap.unscoped.first, :auth_source_ldap => auth_source_ldap_params }, session: set_session_user
